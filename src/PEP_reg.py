...
class PEP(LatentFunctionInference):
    ...
    def inference(self, kern, X, Z, likelihood, Y, mean_function=None, Y_metadata=None):
        ...
        Kmm = kern.K(Z) #Covariance between inducing points Z
        Knn = kern.Kdiag(X) #Diagonal covariance for data points X
        Knm = kern.K(X, Z) #ovariance between data points X and inducing points Z.
        U = Knm

        #factor Kmm
        diag.add(Kmm, self.const_jitter) # add jitter to ensure stability
        Kmmi, L, Li, _ = pdinv(Kmm) #factorize coveriance: pseudo-inverse, cholesky decomposition, its inverse
